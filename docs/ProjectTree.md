# TwoMiceVD プロジェクト構造

## 概要
2台のマウスで仮想デスクトップを切り替えるWindows 11アプリケーションのプロジェクト構造です。1206行の単一ファイルから論理的な14個のファイルに分割し、責任分離の原則に基づいて整理されています。

## フォルダー構造

```
TwoMiceVD/
├── Program.cs                      # アプリケーションエントリポイント
├── Core/                           # コア機能・ビジネスロジック
│   ├── Events.cs                   # DeviceMovedEventArgs (共通イベント引数)
│   ├── VirtualDesktopController.cs # 仮想デスクトップ制御 (H.InputSimulator使用 / WindowsInput互換名前空間)
│   ├── SwitchPolicy.cs             # 切り替えポリシー (閾値・ヒステリシス管理)
│   └── MousePositionManager.cs     # マウスごとの位置記憶・復元機能
├── Input/                          # 入力処理レイヤー
│   └── RawInputManager.cs          # Raw Input API処理 (マウス識別・イベント発生)
├── UI/                             # ユーザーインターフェース
│   ├── TrayUI.cs                   # システムトレイアイコン・コンテキストメニュー
│   ├── PairingDialog.cs            # マウスペアリング設定ダイアログ
│   └── SettingsDialog.cs           # 感度・閾値設定ダイアログ
├── Configuration/                  # 設定管理レイヤー
│   ├── ConfigStore.cs              # 設定ファイルの読み書き (JSON形式)
│   └── ConfigModels.cs             # 設定データクラス (DeviceInfo, VirtualDesktopInfo等)
├── Interop/                        # Windows API相互運用
│   ├── NativeMethods.cs            # P/Invoke メソッド宣言
│   ├── NativeStructs.cs            # P/Invoke 構造体定義
│   ├── NativeEnums.cs              # P/Invoke 列挙型定義
│   └── VirtualDesktopManager.cs    # 仮想デスクトップCOMインターフェース
└── Properties/
    └── AssemblyInfo.cs             # アセンブリ情報
```

## 各ファイルの詳細

### 📁 ルート
- **Program.cs**
  - ApplicationContextを継承したメインクラス
  - すべてのコンポーネントの初期化と連携
  - エラーハンドリングとアプリケーションライフサイクル管理

### 📁 Core/ - コア機能
- **Events.cs**
  - `DeviceMovedEventArgs`: マウス移動イベントの引数クラス
  - デバイスID、移動量(DeltaX, DeltaY)を保持

- **VirtualDesktopController.cs**
  - 仮想デスクトップ間の切り替え制御
  - H.InputSimulator (v1.5.0) を使用したキーボードショートカット送信（WindowsInput互換API）
  - COM APIを使用したGUIDベースの仮想デスクトップ管理
  - ペアリング前検証（メインデスクトップ・枚数チェック）
  - 起動時GUID検証機能
  - Probeウィンドウを使用したデスクトップ識別

- **SwitchPolicy.cs**
  - 移動量の蓄積と閾値判定
  - ヒステリシス（クールダウン）機能
  - ペアリング中の切り替え無効化制御
  - 切り替え結果を返す改善されたインターフェース

- **MousePositionManager.cs**
  - マウスごとのカーソル位置記憶・復元機能
  - マウス切り替え時の位置保存と自動復元
  - クールダウン機能と距離閾値による最適化
  - 単一カーソル前提の設計で競合状態を回避

### 📁 Input/ - 入力処理
- **RawInputManager.cs**
  - Raw Input APIを使用したマウス入力キャプチャ
  - 複数マウスの個別識別
  - HiddenFormによるウィンドウメッセージ受信
  - デバイス移動イベントの発火

### 📁 UI/ - ユーザーインターフェース
- **TrayUI.cs**
  - システムトレイアイコンとコンテキストメニュー
  - ペアリング開始前の仮想デスクトップ構成検証
  - 設定画面、自動起動設定
  - 割り当て反転、バルーン通知機能

- **PairingDialog.cs**
  - マウスA/Bの識別・ペアリング設定
  - リアルタイムマウス識別UI
  - 設定の自動保存

- **SettingsDialog.cs**
  - 感度（移動閾値）とクールダウン時間の調整
  - マウス位置記憶機能のオン/オフ設定
  - アクティブマウス優先モードの有効/無効設定
  - アクティブ猶予時間（50-1000ms）の調整
  - トラックバー・チェックボックス形式のリアルタイム設定UI

### 📁 Configuration/ - 設定管理
- **ConfigStore.cs**
  - JSON形式での設定ファイル読み書き
  - LocalApplicationData配下での設定永続化
  - デバイス-デスクトップのマッピング管理
  - 仮想デスクトップGUIDの保存・検証機能

- **ConfigModels.cs**
  - 設定データのモデルクラス群
  - JSON シリアライゼーション属性付き
  - DeviceInfo, VirtualDesktopInfo, ConfigData等

### 📁 Interop/ - Windows API相互運用
- **NativeMethods.cs**
  - P/Invokeメソッド宣言
  - RegisterRawInputDevices, GetRawInputData等
  - GetCursorPos, SetCursorPos (マウス位置制御用)

- **NativeStructs.cs**
  - Raw Input関連構造体
  - RAWINPUT, RAWMOUSE, RAWINPUTDEVICE等
  - POINT構造体 (マウス座標用)

- **NativeEnums.cs**
  - Raw Input関連列挙型
  - RawInputDeviceFlags, RawInputType等
  - RawMouseFlags, RawInputCommand

- **VirtualDesktopManager.cs**
  - IVirtualDesktopManager COMインターフェース定義
  - VirtualDesktopManagerWrapper ラッパークラス
  - GUIDベースの仮想デスクトップ操作
  - GUID取得・ウィンドウ移動・現在位置判定機能

## アーキテクチャ図

```
┌─────────────────┐
│    Program.cs   │ ← エントリポイント・全体制御
└─────────┬───────┘
          │
    ┌─────▼──────────────────────────────────┐
    │              依存関係                    │
    └─┬───────┬─────────┬──────────────┬─────┘
      │       │         │              │
┌─────▼─┐ ┌──▼───┐ ┌───▼──────┐ ┌────▼──────┐
│ Input │ │ Core │ │    UI    │ │Configuration│
│Layer  │ │Layer │ │  Layer   │ │   Layer     │
└───────┘ └──────┘ └──────────┘ └─────────────┘
      │       │         │              │
      └───────┴─────────┴──────────────┴─────┐
                                             │
                                      ┌─────▼─────┐
                                      │  Interop  │
                                      │   Layer   │
                                      └───────────┘
```

## 技術スタック

- **.NET 8 (Windows Forms)**
- **Raw Input API** - 複数マウス識別
- **H.InputSimulator** - 仮想デスクトップ切り替え（v1.xはWindowsInput名前空間互換）
- **Newtonsoft.Json** - 設定ファイル処理
- **P/Invoke** - Windows API呼び出し

## 主な改善点

### ✅ 保守性の向上
- 単一責任原則の適用
- 関心の分離による影響範囲の限定
- 各ファイルが明確な役割を持つ

### ✅ 可読性の向上
- 論理的なフォルダー構造
- 関連コードのグループ化
- 適切な命名規則

### ✅ 再利用性の向上
- Interop層の分離により他プロジェクトでも利用可能
- 独立性の高いコンポーネント設計

### ✅ テスト容易性
- 各コンポーネントの独立性
- 依存関係の明確化
- モックしやすいインターフェース

### ✅ 型安全性
- Nullable参照型対応
- コンパイル時警告の解消
- より堅牢なコード品質

### ✅ 機能拡張
- **マウス位置記憶機能**: マウス切り替え時に前の位置を記憶・復元
- **アクティブマウス優先モード**: 操作中のマウスを優先し、他方からの入力を一時的に無視
- **詳細設定オプション**: 各機能の有効/無効切り替えと細かい調整が可能
- **仮想デスクトップGUID管理**: COMインターフェースを使用した確実なデスクトップ識別
- **ペアリング前検証**: メインデスクトップ位置・デスクトップ枚数の自動確認
- **起動時GUID検証**: 保存されたデスクトップGUIDの有効性確認と通知
- **後方互換性**: 既存設定ファイルとの互換性を保ちながら新機能を提供

## アクティブマウス優先モードについて

### 動作原理
1. **アクティブ判定**: マウスを動かしたデバイスが「アクティブマウス」として認識される
2. **猶予期間**: アクティブ状態は設定可能な猶予時間（50-1000ms）継続する
3. **排他制御**: アクティブ期間中は他方のマウスからの入力を無視
4. **自動切り替え**: 猶予期間が過ぎると、次に動いたマウスがアクティブになる

### メリット
- **誤動作防止**: 意図しない切り替えやカーソル移動を大幅に削減
- **操作の一貫性**: 一度のマウス操作中は他方の影響を受けない
- **カーソル位置の安定性**: マウス位置復元機能と組み合わせて自然な操作感を実現

## 仮想デスクトップGUID管理について

### 実装概要
Windows COM APIの`IVirtualDesktopManager`インターフェースを使用して、各仮想デスクトップの一意識別子（GUID）を管理しています。

### 主要機能
1. **ペアリング前検証**
   - メイン（左端）デスクトップでの開始を強制
   - 仮想デスクトップ枚数の確認（2枚のみ対応）
   - 検証成功時に自動的にGUIDを設定ファイルに保存

2. **起動時検証**
   - アプリケーション起動時に保存されたGUIDの有効性を非同期で確認
   - GUID無効時はバルーン通知でユーザーに報告
   - 検証失敗時の対処法（再ペアリング）を案内

3. **Probeウィンドウ技術**
   - 不可視の一時ウィンドウを作成してGUID取得
   - システムに影響を与えない安全な実装
   - 確実なデスクトップ識別を実現

### メリット
- **確実性**: インデックスベースより信頼性の高いデスクトップ識別
- **永続性**: Windowsセッション再開後もデスクトップ対応関係を維持
- **ユーザビリティ**: ペアリング失敗要因の事前排除と分かりやすいエラーメッセージ

## ビルド情報

- **ターゲット**: net8.0-windows
- **出力タイプ**: WinExe (Windows実行ファイル)
- **警告レベル**: エラー0個、警告0個（パッケージ互換性の警告なし）
- **NuGetパッケージ**: H.InputSimulator 1.5.0, Newtonsoft.Json 13.0.3
